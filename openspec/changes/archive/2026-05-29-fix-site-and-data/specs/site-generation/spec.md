...
The system SHALL use Eleventy 3.x (ESM) to generate a static website from the `site/` source and `data/users-enriched.json`. It MUST be configured to process the `site/` directory even if it is ignored by the project's `.gitignore`.
